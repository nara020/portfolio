...
export const experiences: Experience[] = [
  {
    ...
    current: true,
    description: {
      ko: "블록체인 B2B 플랫폼, e-Commerce 시스템, 정부 API 연동 등 다양한 프로젝트 수행",
      en: "Blockchain B2B Platform, e-Commerce Systems, Government API Integration projects",
    },
    achievements: {
      ko: [
        "── B2B 공급망 플랫폼 ──",
        "Hyperledger Besu 성능 27배 향상 (25→678 TPS) - ICBTA 2025 Springer 1저자 게재",
        "Groth16 기반 영지식증명(ZKP) 아키텍처 설계 및 Rust 서킷 분석",
        "Solidity 스마트컨트랙트 개발 (UUPS Proxy, ZK 검증, IPFS CID 연동)",
        "TTA 클라우드 적격성 평가 진행 담당",
        "── 아마존/쇼피파이 시스템 ──",
        "2개 개발사 미해결 문제 전량 해결 → 35배 규모 계약 확대",
        "대량 업로드 4배 확장 (2,500개→10,000개+), 크롤링 안정화 95%+",
        "분산 인프라(AWS+Cafe24) 통합 아키텍처 재설계",
        "── 기타 프로젝트 ──",
        "정부 데이터 API(틸코블렛) 연동 - 공급망 참여자 신원 검증",
        "Python 기반 Hyperledger Besu 구축 자동화 툴 개발",
      ],
      en: [
        "── B2B Supply Chain Platform ──",
        "27x TPS improvement on Hyperledger Besu (25→678) - ICBTA 2025 Springer 1st Author",
        "Designed Groth16-based ZKP architecture and analyzed Rust circuits",
        "Solidity smart contracts (UUPS Proxy, ZK verification, IPFS CID)",
        "Led TTA cloud qualification assessment",
        "── Amazon/Shopify System ──",
        "Resolved all issues from 2 vendors → 35x contract scale",
        "4x bulk upload capacity (2,500→10,000+), 95%+ crawling stability",
        "Redesigned distributed infrastructure (AWS+Cafe24)",
        "── Other Projects ──",
        "Government API integration - supply chain participant verification",
        "Python automation tool for Hyperledger Besu deployment",
      ],
    },
...
export const keyMetrics = {
  ...
  papers: { value: "7", desc: { ko: "논문 발표 (5편 1저자)", en: "Papers (5 First Author)" } },
